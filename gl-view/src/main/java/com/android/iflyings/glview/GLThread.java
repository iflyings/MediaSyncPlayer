...
//                            Log.i(TAG, "sending render notification tid=" + getId());
    ...
    public void onPause() {
        synchronized (sGLThreadManager) {
            ...
            while ((!mExited) && (!mPaused)) {
                Log.i(TAG, "onPause waiting for mPaused.");
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
    ...
    public void onResume() {
        synchronized (sGLThreadManager) {
            ...
            while ((!mExited) && mPaused && (!mRenderComplete)) {
                Log.i(TAG, "onResume waiting for !mPaused.");
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
    ...
    public static class ChoreographerRender implements Choreographer.FrameCallback {
        ...
        private WeakReference<GLThread> wrGLThread;
        ...
        public ChoreographerRender(GLThread glThread) {
            this.wrGLThread = new WeakReference<>(glThread);
        }

        @Override
        public void doFrame(long frameTimeNanos) {
            GLThread glThread = wrGLThread.get();
            if (glThread == null) {
                return;
            }
        ...
        public boolean isCanSwap() {
            return canSwap || wrGLThread.get().getRenderMode() == RENDERMODE_WHEN_DIRTY;
        }
    }

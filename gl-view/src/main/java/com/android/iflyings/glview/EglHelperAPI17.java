...
import static com.android.iflyings.glview.EglHelper.formatEglError;
...
public class EglHelperAPI17 implements IEglHelper {
    ...
    public EglContextWrapper start(EglContextWrapper eglContext) {
        ...
        if (mEglContext == null || mEglContext == EGL14.EGL_NO_CONTEXT) {
            Log.d(TAG, "mEglContext:" + mEglContext);
            mEglContext = null;
    ...
    public int swap() {
        if (!EGL14.eglSwapBuffers(mEglDisplay, mEglSurface)) {
            Log.w(TAG, String.format("swap: start get error"));
    ...
    public static void throwEglException(String function, int error) {
        ...
        Log.e(TAG, "throwEglException tid=" + Thread.currentThread().getId() + " " + message);
